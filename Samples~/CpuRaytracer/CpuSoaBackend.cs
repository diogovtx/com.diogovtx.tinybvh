...
{
	/// <summary>CPU backend that traces the scene with a TinyBVH.BvhSoa from a Burst job.</summary>
	public sealed class CpuSoaBackend : IRaytraceBackend
	{
		public string Name => "CPU BVH SoA (Burst)";
		...
		/// <summary>The SoA layout has no opacity micromap API.</summary>
		public bool SupportsOpacityMap => false;
		public float BuildMs { get; private set; }
		public long NodeCount => soa.IsCreated ? soa.UsedNodes : 0;
		...
		public void Build( NativeArray<float4> vertices, uint triCount )
		{
			...
			if ( !soa.IsCreated )
			{
				soa = BvhSoa.Create( Allocator.Persistent );
			}
			Stopwatch stopwatch = Stopwatch.StartNew();
			BvhBuilderOptions.Build( ref soa, Builder, Presplit, ThreadedBuild, vertices, triCount );
			if ( Optimize )
			{
				soa.Optimize();
			}
		...
		private struct RenderJob : IJobParallelFor
		{
			...
			public void Execute( int row )
			{
				...
				for ( int x = 0; x < Width; x++ )
				{
					...
					int steps = Soa.Intersect( ref ray );
