...
{
	/// <summary>
	/// Port of BVH8_CPU::Save and BVH8_CPU::Load: caching a converted layout on disk. Unlike the
	/// C++, which dumps the BVH8_CPU struct itself, the header fields are written one by one, so
	/// the file format does not depend on the struct layout. A loaded layout has no base BVH or
	/// intermediate MBVH&lt;8&gt;, so it cannot be refit, optimized or asked for its SAH cost -
	/// only traversal is available, exactly like the C++, which discards bvh8 on load.
	/// </summary>
	public unsafe partial struct Bvh8Cpu
	{
		/// <summary>File magic; the C++ uses a version/layout header word instead.</summary>
		private const string SaveMagic = "TBVHCS8C";
		...
		public void Save( string path )
		{
			...
			{
				throw new InvalidOperationException( "Bvh8Cpu.Save( .. ), bvh8Cpu was not built." );
			}
			uint flags = 0;
			if ( Refittable )
			{
		...
		/// untouched. On success, Base/Mbvh8 are cleared to default and OwnsSource is false, exactly
		/// like the C++ BVH8_CPU::Load discarding bvh8.
		/// </summary>
		public bool Load( string path, uint expectedTriCount )
		{
			if ( !IsCreated )
			{
				throw new InvalidOperationException( "Bvh8Cpu.Load( .. ), bvh8Cpu was not created." );
			}
			if ( !File.Exists( path ) )
			{
				return false;
			...
			using ( FileStream stream = new FileStream( path, FileMode.Open, FileAccess.Read ) )
			{
				using ( BinaryReader reader = new BinaryReader( stream ) )
				{
					...
					// we can't load the source BVH/MBVH<8> since this layout doesn't own vertex data.
					Base = default;
					Mbvh8 = default;
					OwnsSource = false;
					return true;
		...
		private static void ReadBlock( BinaryReader reader, void* dest, long bytes )
		{
			...
			while ( done < bytes )
			{
				...
				if ( chunk == 0 )
				{
					throw new EndOfStreamException( "Bvh8Cpu.Load( .. ), file ended early." );
				}
